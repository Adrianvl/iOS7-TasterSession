...
- (IBAction)doSaveToFlash:(id)sender;
...
-(NSArray*)arrayOfWisdom
{
    if (_arrayOfWisdom == nil) {
        ...
        NSString* path = [NSBundle pathToFileInDocumentsFolder:@"sayings"];
        self.arrayOfWisdom = [NSKeyedUnarchiver unarchiveObjectWithFile:path];
...
- (IBAction)doSaveToFlash:(id)sender {
    NSString* path = [NSBundle pathToFileInDocumentsFolder:@"sayings"];
    [NSKeyedArchiver archiveRootObject:self.arrayOfWisdom toFile:path];
}
